...
//#region node_modules/.nitro/vite/services/ssr/assets/use-pages-DEg-ttb2.js
function useUserPages(userId, status) {
	return useQuery({
		...
		queryFn: async () => {
			...
			let dbPages = [];
			try {
				let q = supabase.from("pages").select("*").eq("user_id", userId).is("deleted_at", null).order("updated_at", { ascending: false });
...
function usePage(idOrSlug, mode = "id") {
	return useQuery({
		...
		queryFn: async () => {
			try {
				const { data } = await supabase.from("pages").select("*").eq(mode === "id" ? "id" : "slug", idOrSlug).is("deleted_at", null).maybeSingle();
				if (data) return data;
			} catch (e) {
