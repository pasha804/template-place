...
-- Only works if pg_cron extension is available
DO $$ 
BEGIN
...
EXCEPTION 
  WHEN undefined_file THEN 
    RAISE NOTICE 'pg_cron extension not available. You will need to set up a scheduled function or edge function to call cleanup_expired_pages() daily.';
  WHEN OTHERS THEN
    RAISE NOTICE 'Could not set up cron job: %. You can set it up manually later.', SQLERRM;
END $$;
